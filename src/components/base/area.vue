...
<template>
	<div class='main'>
	    <div class="main-search" v-split>
			<div class="main-search__title">行政区管理</div>
...
export default {
	...
	methods: {
		getData() {
			...
			this.axios.post('/data/queryAreaInfoByPage', data).then(res => {
				this.onQuery = false;
				if(200 == res.code) {
		...
		alert(todo, row) {
			...
			if( 'add' == todo) {
				title = '新增行政区信息';
				this.axios.get('/data/getNumberByType', {params: {type: 3}}).then(res => {
					row.number = res.data
				})
			}
			if('edit' == todo) {
